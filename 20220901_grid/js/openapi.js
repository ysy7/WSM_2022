...
const handler = (event) => {
    ...
    let url = `https://open.neis.go.kr/hub/mealServiceDietInfo` + `?KEY=${KEY}` + `&ATPT_OFCDC_SC_CODE=${ATPT_OFCDC_SC_CODE}` + `&SD_SCHUL_CODE=${SD_SCHUL_CODE}` + `&MLSV_YMD=${MLSV_YMD}`;
...
const urlToJSON = (url) => {
    ...
    const showMenu = (jsonString) => {
        ...
        try {
            if (json['mealSerciveDietInfo'][0]['head'][1]['RESULT']['CODE'] == 'INFO-000') {
                //응답이 제대로 왔으면
                //json -> HTML
                console.log(json['mealServiceDietInfo'][1]['row'][0]['DDISH_NM']);
                console.log(json['mealServiceDietInfo'][1]['row'][1]['DDISH_NM']);
                console.log(json['mealServiceDietInfo'][1]['row'][2]['DDISH_NM']);
